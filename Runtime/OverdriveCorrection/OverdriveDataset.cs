...
public class OverdriveDataset {
        ...
        public Int16 this[Color color]
        {
            ...
            set{
                switch(color){
                    ...
                    case OverdriveDataset.Color.G: OverdriveG = value; return;
                    case OverdriveDataset.Color.B: OverdriveB = value; return;
                }
            }
        }
    ...
    public int NEnds{ get => InitialEnds.Length;}
    public int NStarts{ get => Starts.Length;}
    public int NTimemarks{ get => Timemarks.Length;}
    public Sample[,,] Data;
    
    public float[] StartsF { get => Starts.Select(b => ByteToFloat(b)).ToArray(); }
    public float[] InitialEndsF { get => InitialEnds.Select(b => ByteToFloat(b)).ToArray(); }

    public double[] StartsD { get => Starts.Select(b => (double)ByteToFloat(b)).ToArray(); }
    public double[] InitialEndsD { get => InitialEnds.Select(b => (double)ByteToFloat(b)).ToArray(); }
